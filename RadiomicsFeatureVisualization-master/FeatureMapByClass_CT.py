...
class FeatureMapper:
    ...
    def generate_feature_map(self, candidate_img_path, candidate_roi_path, kernelRadius, feature_name_list, store_path):
        ...
        parameter_path = r"D:\zhuomian\pyradiomics\RadiomicsFeatureVisualization-master\FeatureMapByClass_CT.py"
...
def main():
    ...
    features_class_list = ['firstorder','glcm'] #features_class_list是一个包含特征类别的列表。
    cur_file_path = Path(__file__).absolute().parent #cur_file_path 变量最终保存了当前脚本文件的绝对路径的父目录路径。

    # 定义了图像和ROI的路径。
    img_path = cur_file_path / 'data' / 'imagesTr' / 'lung_3.nii.gz'
    roi_path = cur_file_path / 'data' / 'labelsTr' / 'lung_3.nii.gz'
