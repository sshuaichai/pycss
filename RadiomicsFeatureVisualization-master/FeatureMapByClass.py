...
class FeatureMapper:
    ...
    def generate_feature_map(self, candidate_img_path, candidate_roi_path, kernelRadius, feature_name_list, store_path):
        ...
        parameter_path = r"D:\zhuomian\pyradiomics\pyradiomics-master\examples\exampleSettings\exampleCT_lung.yaml"
...
def main():
    ...
    # 定义要提取的特征名称列表features_name_list：在所有特征中选择
    ...
    # 定义了存储特征图的路径。
    store_path = cur_file_path / 'FeatureMapByClass' / 'FeatureMap'
    ...
    # 设置裁剪后的图像路径
    cropped_img_path = cur_file_path / 'FeatureMapByClass' / 'FeatureMap' / 'original_cropped_img.nii.gz'
    # 设置裁剪后的ROI路径
    cropped_roi_path = cur_file_path / 'FeatureMapByClass' / 'FeatureMap' / 'original_cropped_roi.nii.gz'
    # 设置要显示的特征具体名称
    feature_name = 'original_glszm_SizeZoneNonUniformityNormalized'  #在上面的定义的特征列表中选择
    # 设置特征图路径
    feature_map = cur_file_path / 'FeatureMapByClass' / 'FeatureMap' / str(feature_name + '.nrrd')
    # 设置要保存图像的文件路径
    fig_save_path = cur_file_path / 'FeatureMapByClass' / 'FeatureMap' / feature_name
