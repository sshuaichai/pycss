...
使用指定参数Params.yaml
进度日志文件pyrad_log.txt
输出CSV文件radiomics_features_filter.csv：计算原始+滤波特征
'''
...
def main():
    ...
    inputCSV = os.path.join(outPath, 'Dataset022_Lung.csv')  # 提取文件地址
    outputFilepath = os.path.join(outPath, 'radiomics_022_features_filter.csv')
    progress_filename = os.path.join(outPath, 'pyrad_log.txt')
    params = os.path.join(paramspath, 'CT_lung_filter.yaml')
