...
def plot_roc_curve(y_val, y_pred_proba, output_folder):
  ...
  plt.plot(fpr, tpr, color='red', label=f'ROC curve (area = {roc_auc:.2f})',lw=2, alpha=.8) ## 红色来表示平均ROC曲线,加粗用 lw 线宽
  ...
  output_path = os.path.join(output_folder, "Val_tiaocan_roc_curve_validation.tiff")
...
if __name__ == "__main__":
  # 模型和数据集的路径
  model_path = r"D:\zhuomian\pyradiomics\pyradiomics-master\radiomics\data_Conversion_and_extraction\Supervised learning\FeaturesOutput\Train_Output\train_tiaocan_models\Logistic Regression_model.joblib"
