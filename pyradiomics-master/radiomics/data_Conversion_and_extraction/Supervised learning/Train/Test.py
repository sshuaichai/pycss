...
def plot_roc_curve(y_val, y_pred_proba, model_name, output_folder):
    ...
    plt.title(f'Test ROC Curve - {model_name}')
...
def Test_models(model_folder, test_set_path, output_folder):
  # 加载验证集
  ...
  # 保存结果到CSV文件
  results_df.to_csv(os.path.join(output_folder, "Test_results.csv"), index=False)
  print("Train_utils results saved to Test_results.csv")
...
if __name__ == "__main__":
    ...
    test_set_path = r"D:\zhuomian\pyradiomics\pyradiomics-master\radiomics\data_Conversion_and_extraction\Supervised learning\FeaturesOutput\raw_data\processed_test_lasso.csv"
    output_folder = r"D:\zhuomian\pyradiomics\pyradiomics-master\radiomics\data_Conversion_and_extraction\Supervised learning\FeaturesOutput\Test_Output"

    Test_models(model_folder, test_set_path, output_folder)
