...
to_drop = [column for column in upper.columns if any(upper[column] > 0.87)]  # 选择相关性大于0.9的特征进行删除(去除高度共綫性的特徵)
...
plt.savefig(r"D:\zhuomian\pyradiomics\pyradiomics-master\radiomics\data_Conversion_and_extraction\Supervised learning\FeaturesOutput\raw_data\initial_correlation_matrix_heatmap_cox.tiff", format='tiff', dpi=300)
plt.close()
...
plt.savefig(r"D:\zhuomian\pyradiomics\pyradiomics-master\radiomics\data_Conversion_and_extraction\Supervised learning\FeaturesOutput\raw_data\correlation_matrix_heatmap_cox.tiff", format='tiff', dpi=300)
...
for k in range(1, X_scaled_df.shape[1] + 1):  # 遍历所有可能的k值
    ...
    for train_index, test_index in kf.split(X_scaled_df):
        ...
        cph = CoxPHFitter(penalizer=0.6)  # 初始化Cox比例风险模型并添加惩罚项，  # 可以尝试增加这个值，比如0.5或更高，直到警告消失
